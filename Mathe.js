...
class Presenter {
    ...
    setTask() {
        ...
        this.v.renderText(frag.task); //Fragetext in Fragefeld einfügen
        



        //Frage in KaTeX rendern
        //Zuerst MathJax laden, dann rendern
        if (typeof renderMathInElement !== "function") {
            console.error("renderMathInElement is not defined. Make sure MathJax is loaded.");
            return;
        }
        const div =renderMathInElement(document.getElementById("Frage"), {
            delimiters: [
        ...
        //Antworten in KaTeX rendern
        //Zuerst MathJax laden, dann rendern
        if (typeof renderMathInElement !== "function") {
            console.error("renderMathInElement is not defined. Make sure MathJax is loaded.");
            return;
        }
        const div2 =renderMathInElement(document.getElementById("Antwort"), {
            delimiters: [
                {left: '\\(', right: '\\)', display: false}
            ],
            throwOnError: false
...
class View {
    constructor(presenter) {
        ...
        document.getElementById("weiter").addEventListener("click", () => this.p.skipOderWeiterFrage());
    ...
    showResult(feedback, rerg)
    {
        ...
        //Feedback in KaTeX rendern
        //Zuerst MathJax laden, dann rendern
        if (typeof renderMathInElement !== "function") {
            console.error("renderMathInElement is not defined. Make sure MathJax is loaded.");
            return;
        }
        const div3 =renderMathInElement(document.getElementById("Ergebnis-Feedback"), {
            delimiters: [
                {left: '\\(', right: '\\)', display: false}
            ],
            throwOnError: false
  ...
  showZusammenfassung(richtig, falsch, gesamt) {
    ...
    localStorage.setItem("mathe_richtig", richtig);
    localStorage.setItem("mathe_falsch", falsch);
    localStorage.setItem("mathe_gesamt", gesamt);
